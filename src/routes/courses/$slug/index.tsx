...
import { coursePriceLabel, getVideos, getTests } from "@/lib/learning";
import { useEnrollment } from "@/lib/learning-data";
...
export const Route = createFileRoute("/courses/$slug/")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ...
      : [{ title: "Course | Rishi Sidhasamadhi Yoga Foundation" }],
  }),
...
function CoursePage() {
  ...
  const { enrollment, loading } = useEnrollment(course.slug);
  const videos = getVideos(course);
  const tests = getTests(course);
  const price = coursePriceLabel(course);
  const heroImage =
    course.image ?? "https://images.unsplash.com/photo-1518611012118-696072aa579a?w=1920&q=80";
    ...
    <>
      <section className="relative overflow-hidden pt-[90px]">
        ...
        <div className="relative mx-auto max-w-[1400px] px-6 py-28 lg:px-10">
          ...
          <div className="mt-10 flex flex-wrap items-center gap-8">
            {loading ? (
              <span className="rounded-full bg-ivory/40 px-8 py-4 text-[0.78rem] tracking-[0.16em] uppercase text-forest/60">
                Checking your place…
              </span>
            ) : enrollment ? (
              <Link
                ...
                className="rounded-full bg-gold px-8 py-4 text-[0.78rem] tracking-[0.16em] uppercase text-ink transition-colors hover:bg-ivory hover:text-forest"
              >
                Continue learning
              </Link>
            ) : (
            ...
            <div className="flex flex-wrap gap-10 text-ivory/80">
              <Fact label="Tuition" value={price} />
              <Fact label="Duration" value={course.duration} />
              <Fact label="Level" value={course.level} />
              <Fact label="Lessons" value={`${videos.length} recorded`} />
            </div>
          </div>
        </div>
      </section>
      ...
      <Section className="bg-sand/50">
        <div className="grid gap-16 lg:grid-cols-[1fr_1.1fr]">
          <SectionHeader
            eyebrow="Tuition"
            title={<>One payment. The whole path.</>}
            intro="Your enrolment is permanent — the library, the tests, the teacher's replies and your certificate remain yours."
          />
          <div className="sanctuary-card p-10">
            <p className="font-display text-6xl text-forest">{price}</p>
            <p className="mt-2 text-sm text-muted-foreground">
              One-time tuition · lifetime access · scholarships on request
            </p>
            <ul className="mt-9 space-y-4 border-t border-border pt-9">
              {[
                `${videos.length} recorded lessons with notes and highlights`,
                `${tests.filter((t) => t.kind === "written").length} written reflections reviewed by a teacher`,
                "A recorded practice test with personal feedback",
                "Private notes and a question thread on every lesson",
                "Daily practice badges and streak tracking",
                "A certificate of participation on completion",
              ].map((line) => (
                <li key={line} className="flex gap-4 text-sm leading-relaxed text-foreground/80">
            ...
            {!enrollment && !loading && (
              <Link
                to="/courses/$slug/enroll"
                params={{ slug: course.slug }}
                className="mt-10 inline-block rounded-full bg-forest px-8 py-4 text-[0.76rem] tracking-[0.16em] uppercase text-primary-foreground transition-colors hover:bg-gold hover:text-ink"
              >
      ...
      <Section className="bg-forest">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-8 text-[1rem] leading-relaxed text-ivory/75">
            Places are limited so that every student can be known by name. Tuition is {price}.
          </p>
          <Link
            to={enrollment ? "/courses/$slug/videos" : "/courses/$slug/enroll"}
            params={{ slug: course.slug }}
          ...
          >
            {enrollment ? "Continue learning" : "Enrol in this program"}
          </Link>
        </div>
      </Section>
    </>
...
function Fact({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="eyebrow text-ivory/60">{label}</p>
